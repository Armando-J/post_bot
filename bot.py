...
try:
    ...
    import os
    id_canal = os.environ.get('id_canal')
    usercanal = os.environ.get('user_canal')
...
def post_e(temp,id,markup=None):
    ...
    aj(':heavy_check_mark:Audio: <b>{0}</b>\n', temp.post.audio)
    aj(':heavy_check_mark:Idioma: <b>{0}</b>\n', temp.post.idioma)
    aj(':heavy_check_mark:Duración: <b>{0}</b>\n', temp.post.duracion)
    ...
    aj(':heavy_check_mark:Peso: <b>{0}</b>\n', temp.post.peso)
